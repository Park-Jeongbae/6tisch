...
class SchedulingFunctionMSF(SchedulingFunctionBase):
    ...
    def __init__(self, mote):
        ...
        self.num_minimal_cells_rx = [0 for _ in range(1)]
        self.num_minimal_cells_tx = [0 for _ in range(1)]
        self.minimal_cell_utilization  = [0 for _ in range(1)]
    ...
    def _update_minimal_cell_counters(self, cell_opt, used):
        ...
        if used:
            if cell_opt == self.RX_CELL_OPT:
                self.num_minimal_cells_rx[0] += 1
            else:
                self.num_minimal_cells_tx[0] += 1

        if ((self.engine.getAsn()) + (d.MSF_MAX_MINIMAL_NUMCELLS - self.num_minimal_cells_elapsed) * self.settings.tsch_slotframeLength) \
            % (d.MSF_MAX_MINIMAL_NUMCELLS * self.settings.tsch_slotframeLength) != 0:
